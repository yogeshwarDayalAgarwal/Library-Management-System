...
<body>
	...
	<?php  
		...
		{
			$r=mysqli_query($db,"SELECT * FROM admin;");
			$row=mysqli_fetch_assoc($r);
			if($row['username']==$_POST['username'] && $row['email']==$_POST['email'])
			{
				mysqli_query($db,"UPDATE `admin` SET password='$_POST[password]' WHERE username='$_POST[username]' AND email='$_POST[email]';");
				?>
			...
			else
			{
				?>
					<script type="text/javascript">
						alert("The Password is not Updated! Try Again");
					</script>
				<?php
			}
		}
